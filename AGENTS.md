...
| **Client-side idle timeout** | `SESSION_IDLE_TIMEOUT_MINUTES` (server-side, minutes) | 30 | The `useIdleTimer` hook (`src/hooks/useIdleTimer.ts`) tracks real DOM interaction (mouse, keyboard, touch, click, scroll). On timeout it signs out via the dedicated route `POST /api/auth/idle-timeout` (`src/app/api/auth/idle-timeout/route.ts`), which deletes the `sessions` row by cookie token and clears the session cookies — **never** next-auth's client `signOut()` (its server-side failures redirect to `/?error=Configuration` without deleting the session). The value is exposed to the client at runtime via `/api/config` (NOT `NEXT_PUBLIC_*`) so it can be changed without rebuilding. |
...
- **Frozen-page robustness** (`useIdleTimer`): locked screens, suspended PWAs and frozen tabs stop JS timers entirely, and screen lock on desktop does not fire `visibilitychange` (iOS PWAs fire it unreliably) — so lifecycle events alone cannot catch thaw races. The hook instead: (1) makes `recordActivity` **self-defending** — if the *previous* clock is already past the limit, the gesture triggers sign-out instead of resetting the clock; (2) persists the clock to `localStorage` on hide/`pagehide`/throttled activity and **resumes from it** on the next page life (discarded tabs, PWA relaunch); (3) fires `navigator.sendBeacon` before the confirm-fetch (beacons are queued even from frozen pages); (4) retries failed POSTs every 10s tick and hard-navigates to the route's **GET** variant (same deletion + 303 to `/`) after 6 consecutive failures; (5) multi-tab sync via `BroadcastChannel`. Regression tests: `src/hooks/useIdleTimer.test.tsx` (`npm test`).
