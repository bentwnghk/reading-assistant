...
export async function getUserSessions(userId: string): Promise<SessionWithImages[]> {
  ...
  try {
    ...
    return result.rows.map(row => ({
      ...
      summaryGeneratedAt: row.summary_generated_at ?? 0,
      mindMapGeneratedAt: row.mind_map_generated_at ?? 0,
      adaptedTextGeneratedAt: row.adapted_text_generated_at ?? 0,
      simplifiedTextGeneratedAt: row.simplified_text_generated_at ?? 0,
      glossaryGeneratedAt: row.glossary_generated_at ?? 0,
      spellingGameCompletedAt: row.spelling_game_completed_at ?? 0,
      vocabQuizCompletedAt: row.vocab_quiz_completed_at ?? 0,
      readingTestCompletedAt: row.reading_test_completed_at ?? 0,
