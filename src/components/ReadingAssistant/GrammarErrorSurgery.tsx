...
/**
 * Surgery scoring, extracted verbatim from the old inline math in
 * `handleCorrectionSelect` so the breakdown (base / time / streak) can be
 * surfaced to the UI (PointPopup). Do NOT change the formula.
 */
function computeSurgeryPoints(opts: {
  arcade: boolean;
  timeLeft: number;
  ...
  let timeBonus = 0;
  if (opts.arcade) {
    timeBonus = Math.round((opts.timeLeft / 20) * 50);
    pts += timeBonus;
  }
  let streakBonus = 0;
  if (opts.newStreak >= 3) {
...
export default function GrammarErrorSurgery({ onBack }: Props) {
  ...
  const handleCorrectionSelect = useCallback((correction: string) => {
    ...
    if (isCorrect) {
      ...
      const breakdown = computeSurgeryPoints({ arcade: mode === "arcade", timeLeft, newStreak });
      fxSeqRef.current += 1;
      setLastBreakdown({
        ...breakdown,
    ...
    if (gameStatus === "completed" && challenges.length > 0) {
      ...
      if (maxStreak >= 5) {
        logActivity("grammar_hot_streak", {
          ...
          score,
          accuracy,
          details: { mode, game: "surgery", streak: maxStreak },
        });
      }
  ...
  return (
    <div className="relative space-y-4">
      ...
      <div className="relative rounded-xl border p-4 leading-loose">
