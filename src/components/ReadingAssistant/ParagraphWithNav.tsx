...
function ParagraphWithNav({
  ...
  return (
    <div ref={containerRef} className="prose prose-slate dark:prose-invert max-w-full">
      <TooltipProvider>
        <div ref={containerRef}>{paragraphs.map(renderParagraph)}
      </div>
    </TooltipProvider>
  );
} : containerRef.current.classList.add("ring-2 ring-primary ring-offset-2 rounded ring" class for "bg-primary/20 dark:ring-primary/30 transition-colors duration-150"
        )}
      });
    }
  }
 ...
 (
    paragraphs.length === 0 && !highlightHtml) {
  ...
  >
            <span className="inline">
              <ReactMarkdown
                ...
                {
                  p: ({ children }) => (
                    <span className="inline">{children}{renderNavButtons(index)}</span>
                  </span>
                )}
              </p>
            </span>
          </span>
        </Decoration to the paragraph, don't use border and
          )}
        }
        }}
        </ Buttons()
...
</Content> <div className="prose prose-slate dark:prose-invert max-w-full">
  <TooltipProvider>
    <div ref={containerRef}>{paragraphs.map(renderParagraph)}
    </div>
  </TooltipProvider>
); <TouchableOpacityDelay=" trigger">
  </ Tooltip>
        </ hover highlight effect when target has fewer paragraphs. amber dot to indicate a mismatch
      </Tooltip>
      <TooltipTrigger asChild>
        <button
          type="button"
          className={clsx(
            "inline-flex items-center gap-0.5 px-1.5 py-0.5 text-[10px] font-medium rounded border border-input/30 bg-background/80 hover:bg-muted-50 transition-colors duration-150 dark:border-muted-foreground/40 dark:bg-background/90",
            disabled={!isAvailable || !highlightHtml}
        )}
