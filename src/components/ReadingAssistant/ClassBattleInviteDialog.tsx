...
export function ClassBattleInviteDialog() {
  ...
  const handleJoinBattle = useCallback((code: string) => {
    // Carry the chosen invite into the unified battle-lobby dialog (Header
    // entry) and open it. The lobby mounts → connects → the ClassInviteBanner
    // offers a one-click Join (→ joinRoom, queued until connected). Opening the
    // dialog here (instead of router.push("/")) works on every page and avoids
    // the navigate-then-mount race that lost the invite.
    const invite = invites.find((i) => i.roomCode === code);
    if (invite) setClassInvite(invite);
    setShowBattleLobbyDialog(true);
    dismiss(code);
    setShow(false);
  }, [invites, setClassInvite, setShowBattleLobbyDialog, dismiss, setShow]);
