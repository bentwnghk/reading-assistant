                  ...
                  <TableCell className="text-center">
                    {(session.spellingGameBestScore ?? 0) > 0 ? (
                      <button
                        ...
                        className="inline-flex"
                        title={`${t("userManagement.teacherData.viewSpelling")}: ${session.docTitle}`}
                      >
                  ...
                  <TableCell className="text-center">
                    {(session.grammarGameBestScore || 0) > 0 ? (
                      <button
                        ...
                        className="inline-flex"
                        title={`${t("userManagement.teacherData.viewGrammarGame")}: ${session.docTitle}`}
                      >
        ...
        <DialogContent className="max-w-2xl max-h-[85vh] flex flex-col">
          ...
          <div className="flex-1 overflow-y-auto mt-2 space-y-3">
            {viewingSpelling?.results === undefined ? (
                  ...
                  <div className="flex items-start gap-2">
                    ...
                    <div className="flex-1 min-w-0 space-y-1">
                      ...
                      {r.userAnswer ? (
                        ...
                        <p className="text-xs text-red-500 dark:text-red-400 italic">{t("userManagement.teacherData.noAnswer")}</p>
                      )}
                      {!r.correct && (
                        <div className="text-xs px-2 py-1 rounded flex items-center gap-1.5 bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-400 font-medium">
                          <Check className="h-3 w-3 shrink-0" />
              ...
              <div className="text-center py-8 text-muted-foreground">
                {t("userManagement.teacherData.noSpellingResults")}
              </div>
            )}
          </div>
        </DialogContent>
      ...
      <Dialog open={!!viewingGrammarGame} onOpenChange={(open) => { if (!open) setViewingGrammarGame(null) }}>
        <DialogContent className="max-w-2xl max-h-[85vh] flex flex-col">
          <DialogHeader>
            ...
            {viewingGrammarGame?.teacher && (
              <DialogDescription>{viewingGrammarGame.teacher}</DialogDescription>
            )}
          </DialogHeader>
          <div className="flex-1 overflow-y-auto mt-2 space-y-3">
            {viewingGrammarGame?.results === undefined ? (
              ...
              <div className="text-center py-8 text-muted-foreground">
                {t("userManagement.teacherData.noGrammarGameResults")}
              </div>
            )}
          </div>
        </DialogContent>
