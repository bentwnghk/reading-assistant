...
import { generatePreReadingImagePrompt } from "@/constants/readingPrompts";
...
const DAILY_LIMIT_SUBSCRIPTION = parseInt(
  process.env.PRE_READING_IMAGE_DAILY_LIMIT_SUBSCRIPTION ||
    process.env.VISUALIZATION_DAILY_LIMIT_SUBSCRIPTION ||
    "2",
  10
);
const DAILY_LIMIT_FREE = parseInt(
  process.env.PRE_READING_IMAGE_DAILY_LIMIT_FREE ||
    process.env.VISUALIZATION_DAILY_LIMIT_FREE ||
    "1",
  10
);
...
async function callZenMuxApi(prompt: string, model: string): Promise<Response> {
...
async function callGoogleNativeApi(prompt: string, model: string): Promise<Response> {
...
async function callOpenAICompatibleApi(
  ...
  return fetch(`${OPENAI_COMPATIBLE_API_BASE_URL}/v1/chat/completions`, {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
...
async function getDailyLimitInfo(
  ...
  const result = await pool.query(
    `SELECT COUNT(*)::int AS cnt
     FROM activity_logs
     WHERE user_id = $1
       AND activity_type = 'pre_reading_image_generate'
       AND created_at >= $2`,
...
export async function GET(request: NextRequest) {
  try {
    ...
    console.error("Pre-reading image limit check error:", error);
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (limitInfo.remaining <= 0) {
      return NextResponse.json(
        { error: "Daily limit reached", limit: limitInfo.limit, used: limitInfo.used, remaining: 0 },
        { status: 429 }
      );
    }
    ...
    const prompt = generatePreReadingImagePrompt(age, text);
    ...
    if (!imageDataUrl) {
      console.error("All pre-reading image providers failed:", errors);
      return NextResponse.json(
        { error: "Image generation failed", details: errors.join("; ") },
        { status: 502 }
      );
    }
    ...
    console.error("Pre-reading image API error:", error);
    return NextResponse.json({ error: "Internal server error" }, { status: 500 });
  }
}
