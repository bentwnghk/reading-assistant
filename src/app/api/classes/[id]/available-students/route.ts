 ...
 * - Excludes students who are already members of this class (students may belong
 *   to multiple classes, so membership in another class does NOT disqualify).
 */
export async function GET(
  ...
  try {
    const [members, classSchoolId, callerSchoolId] = await Promise.all([
      getClassMembers(id),
      getClassSchoolId(id),
